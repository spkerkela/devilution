...
struct log_cpp_init_1 {
  log_cpp_init_1() { log_cpp_init_value = log_inf; }
} _log_cpp_init_1;
...
//----- (00427A0C) --------------------------------------------------------
void __cdecl log_init_mutex() { InitializeCriticalSection(&sgMemCrit); }

//----- (00427A18) --------------------------------------------------------
void __cdecl j_log_cleanup_mutex() { atexit(log_cleanup_mutex); }

//----- (00427A24) --------------------------------------------------------
void __cdecl log_cleanup_mutex() { DeleteCriticalSection(&sgMemCrit); }
...
void __cdecl log_flush(bool force_close) {
  ...
  if (nNumberOfBytesToWrite) {
    ...
    WriteFile(log_file, lpAddress, nNumberOfBytesToWrite, &NumberOfBytesWritten,
              0);
    nNumberOfBytesToWrite = 0;
  }
...
void *__cdecl log_create() {
  ...
  if (log_not_created) {
    ...
    _snprintf(FileName, 0x104u, "%s%s%02u%02u%02u.ERR", Filename, Buffer,
              _LOWORD(file_info.dwProductVersionMS),
              file_info.dwProductVersionLS >> 16,
              _LOWORD(file_info.dwProductVersionLS));
  }
  v1 = (void *)-1;
  for (pcbBuffer = log_not_created == 0; (signed int)pcbBuffer < 2;
       ++pcbBuffer) {
    v2 = CreateFileA(FileName, GENERIC_WRITE, FILE_SHARE_READ, NULL,
...
void __fastcall log_get_version(VS_FIXEDFILEINFO *file_info) {
  ...
  if (GetModuleFileNameA(0, Filename, 0x104u)) {
    ...
    if (v1) {
      ...
      if (GetFileVersionInfoA(Filename, 0, v2, v3) &&
          VerQueryValueA(v3, "\\", &lpBuffer, &puLen)) {
        v4 = puLen;
        if (puLen >= 0x34)
          v4 = 52;
...
void log_printf(char *pszFmt, ...) {
  ...
  if (lpAddress ||
      (v2 = (char *)VirtualAlloc((LPVOID)lpAddress, 0x1000u, 0x1000u, 4u),
       nNumberOfBytesToWrite = 0, (lpAddress = v2) != 0)) {
...
void __cdecl log_dump_computer_info() {
  ...
  log_printf(
      ...
      "INFO: %s\r\n"
      "\r\n",
      file_info.dwProductVersionMS >> 16, _LOWORD(file_info.dwProductVersionMS),
      file_info.dwProductVersionLS >> 16, _LOWORD(file_info.dwProductVersionLS),
      Buffer, SystemTime.wMonth, SystemTime.wDay, SystemTime.wYear % 100,
