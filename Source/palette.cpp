...
struct palette_cpp_init {
  palette_cpp_init() { palette_cpp_init_value = palette_inf; }
} _palette_cpp_init;
...
void __cdecl palette_init() {
  ...
  LoadSysPal();
  v0 = lpDDInterface->CreatePalette(DDPCAPS_ALLOW256 | DDPCAPS_8BIT,
                                    system_palette, &lpDDPalette, NULL);
  if (v0)
...
void __cdecl LoadSysPal() {
  ...
  if (!fullscreen) {
    ...
    GetSystemPaletteEntries(hDC, iStartIndex, gdwPalEntries,
                            &system_palette[iStartIndex]);
    if (iStartIndex < 256) {
...
//----- (0044923E) --------------------------------------------------------
void __cdecl BlackPalette() { SetFadeLevel(0); }
...
//----- (004493C6) --------------------------------------------------------
bool __cdecl palette_get_colour_cycling() { return color_cycling_enabled; }
