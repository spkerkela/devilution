...
struct wave_cpp_init {
  wave_cpp_init() { wave_cpp_init_value = wave_inf; }
} _wave_cpp_init;
// 4802D4: using guessed type int wave_inf;
// 6ABB34: using guessed type int wave_cpp_init_value;

//----- (00462C7D) --------------------------------------------------------
bool __fastcall WCloseFile(void *file) { return SFileCloseFile(file); }
...
char __fastcall WReadFile(HANDLE hsFile, char *buf, int a3) {
  ...
  v4 = hsFile;
  a2a = 0;
  for (offset = WSetFilePointer(hsFile, 0, 0, 1);;
       WSetFilePointer(v4, offset, 0, 0)) {
...
int __fastcall ReadWaveFile(MEMFILE *pMemFile, WAVEFORMATEX *pwfx, int *a3) {
  ...
  v3 = pwfx;
  v4 = pMemFile;
  if (!ReadMemFile(pMemFile, a2a, 0xCu) || a2a[0] != 'FFIR' ||
      a2a[2] != 'EVAW' || !ReadWaveSection(v4, ' tmf', v9) || v9[0] < 0x10u ||
...
//----- (00462FAE) --------------------------------------------------------
int __fastcall SeekMemFile(MEMFILE *pMemFile, unsigned int lDist,
                           int dwMethod) {
  unsigned int v3; // eax
