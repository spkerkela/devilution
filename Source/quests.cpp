...
QuestData questlist[16] = {
    ...
    {3, 3, DTYPE_CATHEDRAL, 12, 100, 1, 1, QUEST_KING2,
    ...
    {15, 15, DTYPE_CATHEDRAL, 15, 100, 5, 1, QUEST_VILE1,
     "Archbishop Lazarus"}};
char questxoff[7] = {0, -1, 0, -1, -2, -1, -2};
char questyoff[7] = {0, 0, -1, -1, -1, -2, -2};
char *questtrigstr[5] = {"King Leoric's Tomb", "The Chamber of Bone", "Maze",
                         "A Dark Passage", "Unholy Altar"};
...
LABEL_6:
  if (currlevel == quests[15]._qlevel && !setlevel && quests[15]._qvar1 >= 2u) {
    ...
    if (!quests[15]._qvar2 || quests[15]._qvar2 == 2) {
      ...
      AddMissile(quests[15]._qtx, quests[15]._qty, quests[15]._qtx,
                 quests[15]._qty, 0, 65, 0, myplr, 0, 0);
      quests[15]._qvar2 = 1;
  ...
  if (setlevel) {
    if (setlvlnum == quests[13]._qslvl && quests[13]._qactive != 1 &&
        leveltype == quests[13]._qlvltype && nummonsters == 4 &&
  ...
  if (plr[myplr]._pmode == PM_STAND) {
    ...
    do {
      if (currlevel == *(v1 - 2)) {
        v2 = v1[10];
        if (v2) {
          if (*v1 && plr[myplr].WorldX == *(_DWORD *)(v1 + 2) &&
              plr[myplr].WorldY == *(_DWORD *)(v1 + 6)) {
            if (v1[1] != -1)
              setlvltype = v1[1];
...
bool __cdecl ForceQuests() {
  ...
  while (v0 == (QuestStruct *)&quests[15]._qslvl ||
         currlevel != v0[-1]._qslvl || !v0->_qlevel) {
  LABEL_10:
    ++v0;
  ...
  sprintf(infostr, "To %s",
          questtrigstr[(unsigned char)quests[(unsigned char)v0->_qtype]._qslvl -
                       1]);
  cursmx = v1;
  cursmy = v2;
  return 1;
...
bool __fastcall QuestStatus(int i) {
  ...
  if (setlevel || currlevel != quests[i]._qlevel || !quests[i]._qactive ||
...
void __cdecl DrawButcher() {
  DRLG_RectTrans(2 * setpc_x + 19, 2 * setpc_y + 19, 2 * setpc_x + 26,
                 2 * setpc_y + 26);
}
...
void __cdecl ResyncMPQuests() {
  ...
  if (quests[15]._qactive == 1 &&
      currlevel == (unsigned char)quests[15]._qlevel - 1) {
...
void __cdecl ResyncQuests() {
  ...
  if (setlevel && setlvlnum == quests[13]._qslvl && quests[13]._qactive != 1 &&
  ...
  if (QuestStatus(7)) {
    if (quests[7]._qvar1 == 1)
      ObjChangeMapResync(setpc_w + setpc_x - 2, setpc_h + setpc_y - 2,
                         setpc_w + setpc_x + 1, setpc_h + setpc_y + 1);
    if (quests[7]._qvar1 == 2) {
      ObjChangeMapResync(setpc_w + setpc_x - 2, setpc_h + setpc_y - 2,
                         setpc_w + setpc_x + 1, setpc_h + setpc_y + 1);
      ObjChangeMapResync(setpc_x, setpc_y, (setpc_w >> 1) + setpc_x + 2,
                         (setpc_h >> 1) + setpc_y - 2);
    ...
    if (quests[7]._qvar1 == 3) {
      ObjChangeMapResync(setpc_x, setpc_y, setpc_w + setpc_x + 1,
                         setpc_h + setpc_y + 1);
  ...
  if (currlevel == (unsigned char)quests[4]._qlevel + 1 &&
      quests[4]._qactive == 2 && !quests[4]._qvar1) {
    quests[4]._qvar1 = 1;
  ...
  if (currlevel == quests[15]._qlevel && !setlevel &&
