...
struct movie_cpp_init {
  movie_cpp_init() { movie_cpp_init_value = movie_inf; }
} _movie_cpp_init;
...
void __fastcall play_movie(char *pszMovie, bool user_can_close) {
  ...
  if (window_activated) {
    ...
    SVidPlayBegin(v2, 0, 0, 0, 0, loop_movie != 0 ? 0x100C0808 : 0x10280808,
                  &video_stream);
    if (video_stream) {
      do {
...
//----- (0043AE3E) --------------------------------------------------------
LRESULT __stdcall MovieWndProc(HWND hWnd, UINT Msg, WPARAM wParam,
                               LPARAM lParam) {
