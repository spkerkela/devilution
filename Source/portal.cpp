...
//----- (0045118A) --------------------------------------------------------
void __fastcall DeactivatePortal(int i) { portal[i].open = 0; }
...
//----- (00451234) --------------------------------------------------------
void __fastcall SetCurrentPortal(int p) { portalindex = p; }
...
bool __fastcall PosOkPortal(int level, int x, int y) {
  ...
  v3 = &portal[0].x;
  while (!*(v3 - 1) || v3[2] != level ||
         (*v3 != x || v3[1] != y) && (*v3 != x - 1 || v3[1] != y - 1)) {
    v3 += 6;
