...
void __cdecl appfat_cpp_init() { appfat_terminated = 0x7F800000; }

struct j_appfat_cpp_init {
  j_appfat_cpp_init() { appfat_cpp_init(); }
} _j_appfat_cpp_init;
/*
...
char *__fastcall GetErr(int error_code) {
  ...
  if (v2 == 0x0878) {
    ...
    if (!SErrGetErrorStr(error_code, sz_error_buf, 256) &&
        !FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, v1, 0x400u,
                        sz_error_buf, 0x100u, NULL))
...
void __fastcall CenterDlg(HWND hDlg) {
  ...
  ReleaseDC(hWnd, hdc);
  if (!SetWindowPos(hWnd, HWND_TOP, (v6 - v1) / 2, (v3 - v2) / 2, 0, 0,
...
void __fastcall TermDlg(int template_id, int error_code, char *log_file_path,
  ...
  if (DialogBoxParamA(ghInst, (LPCSTR)(unsigned short)v5, ghMainWnd,
                      (DLGPROC)FuncDlg, (LPARAM)dwInitParam) == -1)
    TermMsg("ErrDlg: %d", v5);
  TermMsg(0);
}
...
void __fastcall ErrDlg(template_id template_id, int error_code,
  ...
  DialogBoxParamA(ghInst, (LPCSTR)v6, ghMainWnd, (DLGPROC)FuncDlg,
                  (LPARAM)dwInitParam);
}
...
void __fastcall FileErrDlg(char *error) {
  ...
  if (!v1)
    v1 = &empty_string;
  if (DialogBoxParamA(ghInst, (LPCSTR)0x6A, ghMainWnd, (DLGPROC)FuncDlg,
                      (LPARAM)v1) == -1)
    TermMsg("FileErrDlg");
  TermMsg(0);
}
...
void __fastcall DiskFreeDlg(char *error) {
  ...
  v1 = error;
  FreeDlg();
  if (DialogBoxParamA(ghInst, (LPCSTR)0x6E, ghMainWnd, (DLGPROC)FuncDlg,
                      (LPARAM)v1) == -1)
    TermMsg("DiskFreeDlg");
  TermMsg(0);
}
...
bool __cdecl InsertCDDlg() {
  ...
  ShowCursor(1);
  v0 = DialogBoxParamA(ghInst, (LPCSTR)0x70, ghMainWnd, (DLGPROC)FuncDlg,
                       (LPARAM)&empty_string);
  if (v0 == -1)
    TermMsg("InsertCDDlg");
  ShowCursor(0);
...
void __fastcall DirErrDlg(char *error) {
  ...
  v1 = error;
  FreeDlg();
  if (DialogBoxParamA(ghInst, (LPCSTR)0x72, ghMainWnd, (DLGPROC)FuncDlg,
                      (LPARAM)v1) == -1)
    TermMsg("DirErrorDlg");
  TermMsg(0);
}
