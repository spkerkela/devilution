...
void __fastcall PrintDebugPlayer(bool bNextPlayer) {
  ...
  if (plr[dbgplr].plractive) {
    ...
    NetSendCmdString(1 << myplr, dstr);
    sprintf(dstr, "  Lvl = %i : Change = %i", plr[dbgplr].plrlevel,
            plr[dbgplr]._pLvlChanging);
    NetSendCmdString(1 << myplr, dstr);
    sprintf(dstr, "  x = %i, y = %i : tx = %i, ty = %i", plr[dbgplr].WorldX,
            plr[dbgplr].WorldY, plr[dbgplr]._ptargx, plr[dbgplr]._ptargy);
    NetSendCmdString(1 << myplr, dstr);
    sprintf(dstr, "  mode = %i : daction = %i : walk[0] = %i",
            plr[dbgplr]._pmode, plr[dbgplr].destAction,
            plr[dbgplr].walkpath[0]);
    NetSendCmdString(1 << myplr, dstr);
    sprintf(dstr, "  inv = %i : hp = %i", plr[dbgplr]._pInvincible,
...
void __cdecl PrintDebugQuest() {
  ...
  sprintf(dstr, "Quest %i :  Active = %i, Var1 = %i", dbgqst,
          quests[dbgqst]._qactive, quests[dbgqst]._qvar1);
  NetSendCmdString(1 << myplr, dstr);
  if (++dbgqst == 16)
    dbgqst = 0;
}
...
void __fastcall PrintDebugMonster(int m) {
  ...
  NetSendCmdString(1 << myplr, dstr);
  sprintf(dstr, "Enemy = %i, HP = %i", monster[m]._menemy,
  ...
  sprintf(dstr, "Active List = %i, Squelch = %i", bActive,
          monster[m]._msquelch);
  NetSendCmdString(1 << myplr, dstr);
}
