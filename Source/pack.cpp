...
struct pack_cpp_init {
  pack_cpp_init() { pack_cpp_init_value = pack_inf; }
} _pack_cpp_init;
...
void __fastcall PackItem(PkItemStruct *id, ItemStruct *is) {
  ...
  if (is->_itype == -1) {
    ...
    if (is->IDidx == IDI_EAR) {
      ...
      id->iSeed =
          is->_iName[12] |
          ((is->_iName[11] | ((is->_iName[10] | (is->_iName[9] << 8)) << 8))
           << 8);
      id->bId = is->_iName[13];
      ...
      id->wValue =
          _LOWORD(is->_ivalue) | v3 | ((_LOWORD(is->_iCurs) - 19) << 6);
      id->dwBuff =
          is->_iName[22] |
          ((is->_iName[21] | ((is->_iName[20] | (is->_iName[19] << 8)) << 8))
           << 8);
    } else {
      id->iSeed = is->_iSeed;
...
void __fastcall UnPackItem(PkItemStruct *is, ItemStruct *id) {
  ...
  if (is->idx == -1) {
    ...
    if (is->idx == IDI_EAR) {
      RecreateEar(127, is->iCreateInfo, is->iSeed, is->bId,
                  (unsigned char)is->bDur, (unsigned char)is->bMDur,
                  (unsigned char)is->bCh, (unsigned char)is->bMCh,
                  (unsigned short)is->wValue, is->dwBuff);
    } else {
