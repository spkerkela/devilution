...
char *talkname[9] = {"Griswold", "Pepin", &empty_string, "Ogden", "Cain",
...
void __fastcall DrawSArrows(int a1, int a2) {
  ...
  if (storenumh <= 1)
    ...
    v7 = (*v2 - SStringY[v3] - 24) *
         (1000 * (stextsval + ((v6 - stextup) >> 2)) / (storenumh - 1)) / 1000;
...
//----- (00457FCB) --------------------------------------------------------
void __fastcall AddSTextVal(int y, int val) { stext[y]._sval = val; }

//----- (00457FD8) --------------------------------------------------------
void __fastcall OffsetSTextY(int y, int yo) { stext[y]._syoff = yo; }
  ...
  if (icursW28 == 1) {
    if (icursH28 == 1) {
      ...
      if (plr[myplr].HoldItem._iStatFlag &&
          AllItemsList[plr[v2].HoldItem.IDidx].iUsable) {
        v19 = 0;
        v3 = plr[v2].SpdList;
        do {
          if (v1)
...
void __cdecl S_StartSSell() {
  ...
  for (i = 0; i < plr[myplr]._pNumInv; i++) {
    if (SmithSellOk(i)) {
      sellok = 1;
      qmemcpy(&storehold[storenumh], &plr[myplr].InvList[i],
              sizeof(ItemStruct));
...
void __cdecl S_StartSRepair() {
  ...
  if (plr[myplr].InvBody[0]._itype != -1 &&
      plr[v3].InvBody[0]._iDurability != plr[v3].InvBody[0]._iMaxDur) {
    v12 = 1;
    AddStoreHoldRepair(plr[v3].InvBody, -1);
    v2 = myplr;
  }
  v4 = v2;
  if (plr[v2].InvBody[6]._itype != -1 &&
      plr[v4].InvBody[6]._iDurability != plr[v4].InvBody[6]._iMaxDur) {
    v12 = 1;
    AddStoreHoldRepair(&plr[v4].InvBody[6], -2);
    v2 = myplr;
  }
  v5 = v2;
  if (plr[v2].InvBody[4]._itype != -1 &&
      plr[v5].InvBody[4]._iDurability != plr[v5].InvBody[4]._iMaxDur) {
    v12 = 1;
    AddStoreHoldRepair(&plr[v5].InvBody[4], -3);
    v2 = myplr;
  }
  v6 = v2;
  if (plr[v2].InvBody[5]._itype != -1 &&
      plr[v6].InvBody[5]._iDurability != plr[v6].InvBody[5]._iMaxDur) {
    v12 = 1;
  ...
  if (plr[v2]._pNumInv > 0) {
    ...
    do {
      ...
      if (SmithRepairOk(v0)) {
        v12 = 1;
        AddStoreHoldRepair((ItemStruct *)((char *)&plr[0].InvList[v13] + v7),
                           v0);
        v2 = myplr;
      }
      ++v13;
  ...
  if (v12) {
    ...
    stextscrl = 0;
    sprintf(tempstr, "You have nothing to repair.            Your gold : %i",
            v11);
    AddSText(0, 1, 1u, tempstr, COL_GOLD, 0);
    AddSLine(3);
    AddSLine(21);
...
void __fastcall AddStoreHoldRepair(ItemStruct *itm, int i) {
  ...
  v5 = v3->_ivalue * (100 * (v3->_iMaxDur - v3->_iDurability) / v3->_iMaxDur) /
       100;
  if (!v5) {
    if ((_BYTE)v4 && v3->_iIdentified)
      return;
...
void __cdecl S_StartWSell() {
  ...
  for (i = 0; i < 8; i++) {
    if (plr[myplr].SpdList[i]._itype != -1 && WitchSellOk(~i)) {
      sellok = 1;
      qmemcpy(&storehold[storenumh], &plr[myplr].SpdList[i],
              sizeof(ItemStruct));
...
void __fastcall AddStoreHoldRecharge(ItemStruct itm, int i) {
  ...
  v6 = storehold[v2]._ivalue *
           (100 * (storehold[v2]._iMaxCharges - storehold[v2]._iCharges) /
...
void __cdecl S_StartWRecharge() {
  ...
  v1 = myplr;
  if (plr[myplr].InvBody[4]._itype == ITYPE_STAFF &&
      plr[v1].InvBody[4]._iCharges != plr[v1].InvBody[4]._iMaxCharges) {
    v8 = 1;
  ...
  if (v8) {
    ...
    stextscrl = 0;
    sprintf(tempstr, "You have nothing to recharge.            Your gold : %i",
            v5);
    AddSText(0, 1, 1u, tempstr, COL_GOLD, 0);
    AddSLine(3);
    AddSLine(21);
...
void __cdecl S_StartSIdentify() {
  ...
  if (idok) {
    ...
    stextsval = 0;
    stextsmax = plr[myplr]._pNumInv;
    sprintf(tempstr, "Identify which item?            Your gold : %i",
            plr[myplr]._pGold);
    AddSText(0, 1, 1, tempstr, COL_GOLD, 0);
    AddSLine(3);
    AddSLine(21);
    ...
    stextscrl = 0;
    sprintf(tempstr, "You have nothing to identify.            Your gold : %i",
            plr[myplr]._pGold);
    AddSText(0, 1, 1, tempstr, COL_GOLD, 0);
    AddSLine(3);
    AddSLine(21);
...
void __cdecl S_StartTalk() {
  ...
  do {
    if (*((_BYTE *)v3 - 18) == 2 &&
        *((_DWORD *)&Qtalklist[0]._qinfra + v2 + 16 * talker) != -1 && *v3)
      ++v1;
    v3 += 6;
    ++v2;
  ...
  do {
    if (*((_BYTE *)v0 - 18) == 2 &&
        *((_DWORD *)&Qtalklist[0]._qinfra + v6 + 16 * talker) != -1 && *v0) {
        ...
        if (plr[v3]._pNumInv > 0) {
          do {
            if (v1 <= 0)
              break;
            v19 = 368 * v17 + v18;
            if (*(int *)((char *)&plr[0].InvList[0]._itype + v19) ==
                ITYPE_GOLD) {
...
void __cdecl S_BoyEnter() {
  ...
  if (stextsel == 8 && boyitem._itype != -1 ||
      stextsel == 12 && boyitem._itype == -1) {
    talker = 8;
    stextshold = 12;
    stextlhold = stextsel;
...
void __cdecl S_TalkEnter() {
  ...
  if (stextsel == 22) {
    ...
    do {
      if (*((_BYTE *)v4 - 18) == 2 &&
          *((_DWORD *)&Qtalklist[0]._qinfra + v3 + 16 * talker) != -1 && *v4)
        ++v2;
      v4 += 6;
      ++v3;
