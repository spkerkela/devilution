...
struct nthread_cpp_init_1 {
  nthread_cpp_init_1() { nthread_cpp_init_value = nthread_inf; }
} _nthread_cpp_init_1;
...
//----- (00440DC8) --------------------------------------------------------
void __cdecl nthread_init_mutex() { InitializeCriticalSection(&sgMemCrit); }

//----- (00440DD4) --------------------------------------------------------
void __cdecl nthread_cleanup_mutex_atexit() { atexit(nthread_cleanup_mutex); }

//----- (00440DE0) --------------------------------------------------------
void __cdecl nthread_cleanup_mutex() { DeleteCriticalSection(&sgMemCrit); }
...
int __fastcall nthread_recv_turns(int *pfSendAsync) {
  ...
  *pfSendAsync = 0;
  ...
  if (!v2)
    goto LABEL_11;
  if (SNetReceiveTurns(0, 4, (char **)glpMsgTbl, (unsigned int *)gdwMsgLenTbl,
                       (unsigned long *)player_state)) {
    if (!byte_679758) {
      byte_679758 = 1;
...
//----- (00440F56) --------------------------------------------------------
void __cdecl nthread_set_turn_upper_bit() { dword_679754 = 0x80000000; }
// 679754: using guessed type int dword_679754;
...
void __fastcall nthread_start(bool set_turn_upper_bit) {
  ...
  if (caps.maxplayers > 4u)
    caps.maxplayers = 4;
  v5 = (3 * (caps.bytessec * (unsigned int)(unsigned char)byte_679704 / 0x14) >>
        2) /
       caps.maxplayers;
  gdwNormalMsgSize = v5;
  if (v5 < 0x80) {
  ...
  if ((unsigned char)gbMaxPlayers > 1u) {
    ...
    byte_679734 = 1;
    sghThread = (HANDLE)_beginthreadex(NULL, 0, nthread_handler, NULL, 0,
                                       &glpNThreadId);
