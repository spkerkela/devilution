...
struct init_cpp_init {
  init_cpp_init() { init_cpp_init_value = init_inf; }
} _init_cpp_init;
...
void __fastcall init_run_office(char *dir) {
  ...
  if (v2 != (HANDLE)-1) {
    do {
      ...
      } else if (!_strcmpi(FindFileData.cFileName,
                           "Microsoft Office Shortcut Bar.lnk")) {
        v4 = GetDesktopWindow();
        ShellExecuteA(v4, "open", FindFileData.cFileName, &empty_string, v1,
                      SW_SHOWNORMAL);
      }
...
void __fastcall init_disable_screensaver(bool disable) {
  ...
  if (!RegOpenKeyExA(HKEY_CURRENT_USER, "Control Panel\\Desktop", 0,
                     KEY_READ | KEY_WRITE, &phkResult)) {
    if (v6) {
      cbData = 16;
      if (!RegQueryValueExA(phkResult, "ScreenSaveActive", 0, &Type,
                            (LPBYTE)Data, &cbData))
    ...
    Data[1] = 0;
    Data[0] = (v1 != 0) + '0';
    RegSetValueExA(phkResult, "ScreenSaveActive", 0, REG_SZ, (const BYTE *)Data,
                   2u);
...
void __cdecl init_create_window() {
  ...
  wcex.hIconSm = (HICON)LoadImageA(ghInst, (LPCSTR)0x65, IMAGE_ICON, 16, 16,
                                   LR_DEFAULTCOLOR);
  ...
  if (GetSystemMetrics(SM_CYSCREEN) >= 480)
    ...
    nHeight = 480;
  hWnd = CreateWindowExA(0, "DIABLO", "DIABLO", WS_POPUP, 0, 0, nWidth, nHeight,
...
void __cdecl init_archives() {
  ...
  while (1) {
#endif
    diabdat_mpq = init_test_access(diabdat_mpq_path, "\\diabdat.mpq",
                                   "DiabloCD", 1000, 1);
#ifdef COPYPROT
    if (diabdat_mpq)
      break;
  ...
  WCloseFile(a1);
  patch_rt_mpq = init_test_access(patch_rt_mpq_path, "\\patch_rt.mpq",
                                  "DiabloInstall", 2000, 0);
}
    ...
    if (v0) {
      ...
      if (GetFileVersionInfoA(diablo_exe_path, 0, v1, v2)) {
        if (VerQueryValueA(v2, "\\", (LPVOID *)&lpBuffer, &uBytes))
          sprintf(gszVersionNumber, "version %d.%d.%d.%d",
