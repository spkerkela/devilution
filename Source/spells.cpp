...
SpellData spelldata[37] = {{SPL_NULL,
                            0,
                            STYPE_FIRE,
                            NULL,
                            NULL,
...
//----- (004575D5) --------------------------------------------------------
void __fastcall CastSpell(int id, int spl, int sx, int sy, int dx, int dy,
                          int caster, int spllvl) {
  int v8;             // eax
  signed int v9;      // edi
...
LABEL_7:
  ...
  if (*v10) {
    do {
      if (v9 >= 3)
        break;
      AddMissile(sx, sy, dx, dy, midir, (unsigned char)v10[v9++], caster, ids,
                 0, spllvl);
    } while (v10[v9]);
  }
  if (*v10 == MIS_TOWN)
...
void __fastcall DoResurrect(int pnum, int rid) {
  ...
  if ((_BYTE)rid != LOBYTE(-1))
    AddMissile(plr[rid].WorldX, plr[rid].WorldY, plr[rid].WorldX,
               plr[rid].WorldY, 0, MIS_RESURRECTBEAM, 0, pnum, 0, 0);
  if (v3 == myplr)
