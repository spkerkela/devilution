...
int sgnMusicTrack = 6;
char *sgszMusicTracks[6] = {"Music\\DTowne.wav", "Music\\DLvlA.wav",
                            "Music\\DLvlB.wav",  "Music\\DLvlC.wav",
                            "Music\\DLvlD.wav",  "Music\\Dintro.wav"};
RECT8 QSRect[2] = {{{15, -16}, {15, -16}},
                   {{30, -31}, {30, -31}}}; /* psx version? */

//----- (00456CC0) --------------------------------------------------------
struct sound_cpp_init {
  sound_cpp_init() { sound_cpp_init_value = sound_inf; }
} _sound_cpp_init;
...
void __fastcall snd_update(bool bStopAll) {
  ...
  do {
    v3 = DSBs[v2];
    if (v3 && (v1 || v3->GetStatus(&v4) ||
               v4 != DSBSTATUS_PLAYING)) // FIX_ME: double check
    {
      DSBs[v2]->Stop();
      DSBs[v2]->Release();
      DSBs[v2] = 0;
...
bool __fastcall snd_playing(TSnd *pSnd) {
  ...
  if (pSnd && (v1 = pSnd->DSB) != 0 && !v1->GetStatus(&v3)) {
...
void __fastcall sound_create_primary_buffer(int music_track) {
  ...
  if (sglpDSB) {
    ...
    a2.nChannels = 2;
    a2.nBlockAlign =
        2 * a2.wBitsPerSample / 8; // a2.nChannels * x / 8 (BUG_FIX: per MSDN)
...
int __fastcall sound_DirectSoundCreate(GUID *guid, IDirectSound **DS,
  ...
  return ((int(__stdcall *)(GUID *, IDirectSound **, int))v5)(v8, v3,
                                                              always_null);
}
