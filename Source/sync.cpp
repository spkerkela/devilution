...
int __fastcall sync_all_monsters(TSyncHeader *packet, int size) {
  ...
  if (nummonsters >= 1 && (unsigned int)size >= 0x2B) {
    ...
    if (nummonsters > 0) {
      do {
        if (v6 < 5 ||
            (v4 >= 2 || !sync_monster_active2(v5)) && !sync_monster_active(v5))
          break;
        v3->wLen += 5;
        v6 -= 5;
        ++v5;
...
int __fastcall sync_monster_active(TSyncMonster *packet) {
  ...
  do {
    v4 = monstactive[v2];
    if ((unsigned short)sync_word_6AA708[v4] < v1 &&
        (unsigned short)sync_word_6AA89C[v4] < 0xFFFEu) {
  ...
  if (numitems <= 0) {
    ...
    *(_WORD *)(&pItem->bItemY + 1) = item[v3].IDidx;
    if (item[v3].IDidx == IDI_EAR) {
      ...
      _LOBYTE(v5) = 0;
      *(unsigned short *)((char *)&pItem->wItemIndx + 1) =
          item[v3]._iName[8] | v4;
      *(_DWORD *)((char *)&pItem->wItemCI + 1) =
          item[v3]._iName[12] |
          ((item[v3]._iName[11] |
            ((item[v3]._iName[10] | (item[v3]._iName[9] << 8)) << 8))
           << 8);
      ...
      *(_WORD *)&pItem->bItemMDur = _LOWORD(item[v3]._ivalue) | v5 |
                                    ((_LOWORD(item[v3]._iCurs) - 19) << 6);
      *(_DWORD *)&pItem->bItemMCh =
          item[v3]._iName[22] |
          ((item[v3]._iName[21] |
            ((item[v3]._iName[20] | (item[v3]._iName[19] << 8)) << 8))
           << 8);
    } else {
      *(unsigned short *)((char *)&pItem->wItemIndx + 1) =
          item[v3]._iCreateInfo;
...
void __fastcall sync_monster_data(int pnum, TSyncMonster *packet) {
  ...
  if (monster[v5]._mhitpoints) {
    ...
    if (v7 >= v8 && (v7 != v8 || md <= myplr)) {
      ...
      if (monster[v5]._mfutx != v9 ||
          monster[v5]._mfuty != (unsigned char)v2->_my) {
        v10 = monster[v5]._mmode;
        if (v10 != MM_CHARGE && v10 != MM_STONE) {
          v11 = abs(monster[v5]._mx - v9);
          v12 = abs(monster[v5]._my - (unsigned char)v2->_my);
          if (v11 > 2 || v12 > 2) {
            if (dMonster[0][(unsigned char)v2->_my +
                            112 * (unsigned char)v2->_mx]) {
            LABEL_23:
              decode_enemy(v4, (unsigned char)v2->_menemy);
              return;
            }
            M_ClearSquares(v4);
            dMonster[0][(unsigned char)v2->_my + 112 * (unsigned char)v2->_mx] =
                v4 + 1;
            monster[v5]._mx = (unsigned char)v2->_mx;
            monster[v5]._my = (unsigned char)v2->_my;
            decode_enemy(v4, (unsigned char)v2->_menemy);
            v16 = GetDirection((unsigned char)v2->_mx, (unsigned char)v2->_my,
            ...
            if (v13 >= MM_WALK && v13 <= MM_WALK3)
              goto LABEL_23;
            v14 = GetDirection(monster[v5]._mx, monster[v5]._my,
                               (unsigned char)v2->_mx, (unsigned char)v2->_my);
            mda = v14;
