...
struct dx_cpp_init_1 {
  dx_cpp_init_1() { dx_cpp_init_value = dx_inf; }
} _dx_cpp_init_1;
...
//----- (0041537C) --------------------------------------------------------
void __cdecl dx_init_mutex() { InitializeCriticalSection(&sgMemCrit); }

//----- (00415388) --------------------------------------------------------
void __cdecl dx_cleanup_mutex_atexit() { atexit(dx_cleanup_mutex); }

//----- (00415394) --------------------------------------------------------
void __cdecl dx_cleanup_mutex() { DeleteCriticalSection(&sgMemCrit); }
...
void __fastcall dx_init(HWND hWnd) {
  ...
  fullscreen = 1;
  v4 = lpDDInterface->SetCooperativeLevel(
      v1, DDSCL_EXCLUSIVE | DDSCL_ALLOWREBOOT | DDSCL_FULLSCREEN);
  ...
  SDrawManualInitialize(hWnda, lpDDInterface, lpDDSPrimary, 0, 0, lpDDSBackBuf,
                        lpDDPalette, 0);
}
...
void __cdecl dx_create_back_buffer() {
  ...
  v4.dwSize = 108;
  v4.dwFlags =
      DDSD_PIXELFORMAT | DDSD_PITCH | DDSD_WIDTH | DDSD_HEIGHT | DDSD_CAPS;
...
HRESULT __fastcall dx_DirectDrawCreate(GUID *guid, IDirectDraw **DD,
  ...
  return ((int(__stdcall *)(GUID *, IDirectDraw **, void *))v5)(v8, v3,
                                                                unknown);
}
