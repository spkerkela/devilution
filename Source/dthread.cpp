...
struct dthread_cpp_init_1 {
  dthread_cpp_init_1() { dthread_cpp_init_value = dthread_inf; }
} _dthread_cpp_init_1;
...
//----- (004150B2) --------------------------------------------------------
void __cdecl dthread_init_mutex() { InitializeCriticalSection(&sgMemCrit); }

//----- (004150BE) --------------------------------------------------------
void __cdecl dthread_cleanup_mutex_atexit() { atexit(dthread_cleanup_mutex); }

//----- (004150CA) --------------------------------------------------------
void __cdecl dthread_cleanup_mutex() { DeleteCriticalSection(&sgMemCrit); }
...
void __cdecl dthread_start() {
  ...
  if (gbMaxPlayers != 1) {
    ...
    byte_52A508 = 1;
    sghThread = (HANDLE)_beginthreadex(NULL, 0, dthread_handler, NULL, 0,
                                       &glpDThreadId);
...
unsigned int __stdcall dthread_handler(void *a1) {
  ...
  while (byte_52A508) {
    if (!sgpInfoHead &&
        WaitForSingleObject(sghWorkToDoEvent, 0xFFFFFFFF) == -1) {
      v1 = GetLastErr();
      TermMsg("dthread4:\n%s", v1);
    }
    EnterCriticalSection(&sgMemCrit);
    ...
    if (v2) {
      v3 = v2->dwSpaceLeft;
      if (v3 != 4)
        multi_send_zero_packet(v3, v2->data[0], &v2->data[8],
                               *(_DWORD *)&v2->data[4]);
