...
//----- (004493D4) --------------------------------------------------------
int __fastcall FindPath(bool(__fastcall *PosOk)(int, int, int), int PosOkArg,
                        int sx, int sy, int dx, int dy, char *path) {
  PATHNODE *v8;  // esi
  char v9;       // al
  PATHNODE *v11; // eax
  int result;    // eax
  ...
  if (*(_DWORD *)v14) {
    while (1) {
      ...
      if (v15 >= 25)
        break;
      pnode_vals[++v15] =
          path_directions[3 * (v13->y - *(_DWORD *)(*(_DWORD *)v14 + 8)) -
                          *(_DWORD *)(*(_DWORD *)v14 + 4) + 4 + v13->x];
      v13 = *(PATHNODE **)v14;
      v14 = *(_DWORD *)v14 + 12;
      if (!*(_DWORD *)v14) {
        v16 = v15 == 25;
...
PATHNODE *__cdecl GetNextPath() {
  ...
  if (result) {
    *(_DWORD *)(*(_DWORD *)&path_2_nodes[0].f + 48) =
        (unsigned int)result->NextNode;
...
//----- (004495ED) --------------------------------------------------------
int __fastcall path_get_path(bool(__fastcall *PosOk)(int, int, int),
                             int PosOkArg, PATHNODE *pPath, int x, int y) {
  int v5; // eax
  int dx; // esi
  int dy; // edi
...
//----- (0044966F) --------------------------------------------------------
int __fastcall path_parent_path(PATHNODE *pPath, int dx, int dy, int sx,
                                int sy) {
  PATHNODE *v5;          // edi
  int v6;                // ebx
  PATHNODE *v7;          // esi
...
void __fastcall path_set_coords(PATHNODE *pPath) {
  ...
  while (gdwCurPathStep) {
    ...
    for (i = 0; i < 8; i++) {
      ...
      if (PathOld->g + path_check_equal(PathOld, PathAct->x, PathAct->y) <
          PathAct->g) {
