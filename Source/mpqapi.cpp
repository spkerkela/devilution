...
struct mpqapi_cpp_init {
  mpqapi_cpp_init() { mpqapi_cpp_init_value = mpqapi_inf; }
} _mpqapi_cpp_init;
...
bool __fastcall mpqapi_reg_load_modification_time(char *dst, int size) {
  ...
  v3 = dst;
  memset(dst, 0, size);
  if (!SRegLoadData("Diablo", "Video Player ", 0, (unsigned char *)v3, v2,
...
_BLOCKENTRY *__fastcall mpqapi_new_block(int *block_index) {
  ...
  result = sgpBlockTbl;
  v2 = 0;
  while (result->offset || result->sizealloc || result->flags ||
         result->sizefile) {
    ++v2;
    ++result;
    if (v2 >= 0x800) {
...
//----- (0043B153) --------------------------------------------------------
int __fastcall mpqapi_get_hash_index(short index, int hash_a, int hash_b,
                                     int locale) {
  int v4;         // ecx
  signed int v5;  // eax
  signed int v6;  // edx
  ...
  for (i = v4;; i = (i + 1) & 0x7FF) {
    ...
    if (!v6)
      return -1;
    if (v7->hashcheck[0] == v10 && v7->hashcheck[1] == hash_b &&
        v7->lcid == locale && v8 != -2)
      break;
    v4 = (i + 1) & 0x7FF;
  }
...
//----- (0043B1BD) --------------------------------------------------------
void __fastcall mpqapi_remove_hash_entries(bool(__stdcall *fnGetName)(int,
                                                                      char *)) {
...
//----- (0043B23D) --------------------------------------------------------
_BLOCKENTRY *__fastcall mpqapi_add_file(char *pszName, _BLOCKENTRY *pBlk,
                                        int block_index) {
  char *v3;         // edi
  short v4;         // si
  int v5;           // ebx
...
//----- (0043B317) --------------------------------------------------------
bool __fastcall mpqapi_write_file_contents(char *pszName, char *pbData,
                                           int dwLen, _BLOCKENTRY *pBlk) {
  char *v4;                    // esi
  char *v5;                    // eax
  ...
  while (v8) {
    ...
    if (!v18) {
      ...
      if (!WriteFile(sghArchive, pBlk, nNumberOfBytesToWrite,
  ...
  *(&pBlk->offset + v18) = destsize;
  if (SetFilePointer(sghArchive, -destsize, NULL, FILE_CURRENT) == -1 ||
      !WriteFile(sghArchive, pBlk, nNumberOfBytesToWrite,
                 &nNumberOfBytesToWrite, 0) ||
      SetFilePointer(sghArchive, destsize - nNumberOfBytesToWrite, NULL,
                     FILE_CURRENT) == -1) {
  LABEL_25:
    if (pBlk)
      mem_free_dbg(pBlk);
                                    ...
                                    int dwChar) // OpenMPQ
{
  ...
  save_archive_open = 0;
  sghArchive = CreateFileA(v3, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                           OPEN_EXISTING, v6, NULL);
  if (sghArchive == (HANDLE)-1) {
    sghArchive = CreateFileA(
        lpFileName, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
        v6 | (v4 != 0 ? FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_HIDDEN : 0),
        NULL);
    if (sghArchive == (HANDLE)-1)
      return 0;
    save_archive_open = 1;
...
//----- (0043B791) --------------------------------------------------------
bool __fastcall mpqapi_parse_archive_header(
    TMPQHeader *pHdr, int *pdwNextFileStart) // ParseMPQHeader
{
  int *v2;                 // ebp
  TMPQHeader *v3;          // esi
  DWORD v4;                // eax
  ...
  *v2 = v4;
  if (v4 == -1 || v4 < 0x68 ||
      ...
      NumberOfBytesRead != 104 || v3->dwID != '\x1AQPM' ||
      v3->dwHeaderSize != 32 || v3->wFormatVersion > 0u ||
      v3->wSectorSize != 3 || v3->dwArchiveSize != v5 ||
      v3->dwHashTablePos != 32872 || v3->dwBlockTablePos != 104 ||
      v3->dwHashTableSize != 2048 || v3->dwBlockTableSize != 2048) {
    if (SetFilePointer(sghArchive, 0, NULL, FILE_BEGIN) == -1 ||
        !SetEndOfFile(sghArchive))
      return 0;
    memset(v3, 0, 0x68u);
    v3->dwID = '\x1AQPM';
    v3->dwHeaderSize = 32;
...
//----- (0043B882) --------------------------------------------------------
void __fastcall mpqapi_close_archive(char *pszArchive, bool bFree,
                                     int dwChar) // CloseMPQ
{
  char *v3;        // esi
  _BLOCKENTRY *v4; // ecx
...
//----- (0043B970) --------------------------------------------------------
void __fastcall mpqapi_flush_and_close(char *pszArchive, bool bFree,
                                       int dwChar) {
  if (sghArchive != (HANDLE)-1) {
    if (save_archive_modified) {
      if (mpqapi_can_seek()) {
...
bool __cdecl mpqapi_write_header() // WriteMPQHeader
{
  ...
  if (SetFilePointer(sghArchive, 0, NULL, FILE_BEGIN) != -1 &&
      WriteFile(sghArchive, &fhdr, 0x68u, &NumberOfBytesWritten, 0))
