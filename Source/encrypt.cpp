...
//----- (00415F43) --------------------------------------------------------
void __fastcall encrypt_decrypt_block(void *block, int size,
                                      int key) // DecryptMPQBlock
{
  unsigned int v3; // edx
  int v4;          // eax
  unsigned int v5; // edi
...
//----- (00415F8F) --------------------------------------------------------
void __fastcall encrypt_encrypt_block(void *block, int size,
                                      int key) // EncryptMPQBlock
{
  unsigned int v3; // edx
  int v4;          // eax
  unsigned int v5; // edi
...
int __fastcall encrypt_compress(void *buf, int size) // MPQCompress_PKWare
{
  ...
  param.pbSize = v3;
  dsize = 4096;
  implode(encrypt_pkware_read, encrypt_pkware_write, ptr, &param, &type,
          &dsize);
...
//----- (00416133) --------------------------------------------------------
unsigned int __cdecl encrypt_pkware_read(char *buf, unsigned int *size,
                                         void *param) // ReadPKWare
{
...
//----- (00416167) --------------------------------------------------------
void __cdecl encrypt_pkware_write(char *buf, unsigned int *size,
                                  void *param) // WritePKWare
{
  TDataInfo *pInfo = (TDataInfo *)param;
...
//----- (0041618E) --------------------------------------------------------
void __fastcall encrypt_decompress(void *param, int recv_size,
                                   int dwMaxBytes) // MPQDecompress_PKWare
{
  ...
  info.pbOutBuff = v5;
  info.pbSize = v4;
  explode(encrypt_pkware_read, encrypt_pkware_write, ptr, &info);
