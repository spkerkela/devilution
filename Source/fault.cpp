...
//----- (004182B7) --------------------------------------------------------
void __cdecl exception_install_filter() { exception_set_filter(); }

//----- (004182C1) --------------------------------------------------------
void __cdecl j_exception_init_filter() { atexit(exception_init_filter); }

//----- (004182CD) --------------------------------------------------------
void __cdecl exception_init_filter() { exception_set_filter_ptr(); }
...
LONG __stdcall TopLevelExceptionFilter(
  ...
  v2 = exception_get_error_type(ExceptionInfo->ExceptionRecord->ExceptionCode,
                                v5, 0x104u);
  log_printf("Exception code: %08X %s\r\n", v1->ExceptionCode, v2);
  exception_unknown_module(v1->ExceptionAddress, String1, 260, (int)&a4,
                           (int)&a5);
  log_printf("Fault address:\t%08X %02X:%08X %s\r\n", v1->ExceptionAddress, a4,
             a5, String1);
  v3 = ExceptionInfo->ContextRecord;
  log_printf("\r\nRegisters:\r\n");
  log_printf("EAX:%08X\r\nEBX:%08X\r\nECX:%08X\r\nEDX:%08X\r\nESI:%08X\r\nEDI:%"
             "08X\r\n",
             v3->Eax, v3->Ebx, v3->Ecx, v3->Edx, v3->Esi, v3->Edi);
...
//----- (00418518) --------------------------------------------------------
void __fastcall exception_unknown_module(LPCVOID lpAddress, LPSTR lpString1,
                                         int iMaxLength, int a4, int a5) {
  int v6;                                  // eax
  char *v7;                                // eax
  unsigned int v8;                         // edi
  unsigned int v9;                         // esi
...
//----- (00418688) --------------------------------------------------------
char *__fastcall exception_get_error_type(DWORD dwMessageId, LPSTR lpString1,
                                          DWORD nSize) {
  CHAR *v3;       // esi
  const CHAR *v4; // eax
  CHAR *v5;       // ST10_4
