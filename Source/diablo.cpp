...
char *spszMsgTbl[4] = {"I need help! Come Here!", "Follow me.",
                       "Here's something for you.", "Now you DIE!"}; // weak
char *spszMsgKeyTbl[4] = {"F9", "F10", "F11", "F12"};                // weak

//----- (004087B6) --------------------------------------------------------
struct diablo_cpp_init {
  diablo_cpp_init() { diablo_cpp_init_value = diablo_inf; }
} _diablo_cpp_init;
...
int __fastcall diablo_init_menu(int a1, int bSinglePlayer) {
  ...
  while (1) {
    ...
    if ((v3 || !*(_DWORD *)&gbValidSaveFile) &&
            (InitLevels(), InitQuests(), InitPortals(), InitDungMsgs(myplr),
...
void __fastcall run_game_loop(int uMsg) {
  ...
  (HWND, UINT, WPARAM, LPARAM); // [esp+Ch] [ebp-20h]
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  ghInst = v4;
  if (RestrictedTest())
    ErrDlg(TEMPLATE_ERR_RESTRICTED, 0, "C:\\Src\\Diablo\\Source\\DIABLO.CPP",
           877);
  if (ReadOnlyTest()) {
...
void __fastcall diablo_reload_process(HMODULE hModule) {
  ...
  *Filename = empty_string;
  ...
  if (sinf.dwPageSize < 4096)
    dwSize = 4096;
  hMap = CreateFileMappingA((HANDLE)0xFFFFFFFF, NULL,
                            SEC_COMMIT | PAGE_READWRITE, 0, dwSize, Name);
    ...
    if (v4) {
      if (v3) {
        *v4 = -1;
        ...
        si.cb = sizeof(si);
        CreateProcessA(Filename, NULL, NULL, NULL, FALSE,
...
//----- (0040905E) --------------------------------------------------------
LRESULT __stdcall DisableInputWndProc(HWND hWnd, int uMsg, int wParam,
                                      int lParam) {
  bool v5; // zf
  ...
  if (uMsg <= (unsigned int)WM_LBUTTONDOWN) {
    if (uMsg != WM_LBUTTONDOWN) {
      if (uMsg >= (unsigned int)WM_KEYFIRST &&
          (uMsg <= (unsigned int)WM_CHAR || uMsg == WM_SYSKEYDOWN ||
           uMsg == WM_SYSCOMMAND || uMsg == WM_MOUSEFIRST)) {
        return 0;
      }
...
bool __fastcall LeftMouseDown(int a1) {
  ...
  if (lvlbtndown)
    return 0;
  if (leveltype) {
    v7 = abs(plr[myplr].WorldX - cursmx) < 2 &&
         abs(plr[myplr].WorldY - cursmy) < 2;
    _HIWORD(v8) = _HIWORD(pcurs);
    if (pcursitem != -1 && pcurs == 1 && v1 != 5) {
      _LOWORD(v8) = pcursitem;
      NetSendCmdLocParam1(1u, (invflag == 0) + CMD_GOTOGETITEM, cursmx, cursmy,
                          v8);
    LABEL_96:
      if (pcursitem != -1)
        return 0;
    ...
    if (pcursobj != -1) {
      if (v1 != 5 || v7 && (v7 = 120 * pcursobj,
                            *((_BYTE *)&object[0]._oBreak + v7) == 1)) {
        _LOWORD(v7) = pcursobj;
...
void __cdecl RightMouseDown() {
  if (!gmenu_exception() && sgnTimeoutCurs == CURSOR_NONE && PauseMode != 2 &&
    ...
    if (doomflag) {
      ...
      if (spselflag) {
        SetSpell();
                 ...
                 (!sbookflag || MouseX <= 320) && !TryIconCurs() &&
...
void __fastcall diablo_hotkey_msg(int dwMsg) {
  ...
  if (gbMaxPlayers != 1) {
    ...
    GetPrivateProfileStringA("NetMsg", spszMsgKeyTbl[v1], spszMsgTbl[v1],
                             ReturnedString, 0x50u, Filename);
      ...
      LABEL_113:
        ...
        if (sgnTimeoutCurs == CURSOR_NONE && !dropGoldFlag) {
          ...
          if (PauseMode != 2) {
            ...
            case VK_F3:
              if (pcursitem != -1) {
                sprintf(tempstr, "IDX = %i  :  Seed = %i  :  CF = %i",
                        item[pcursitem].IDidx, item[pcursitem]._iSeed,
...
void __fastcall PressChar(int vkey) {
  ...
  v1 = vkey;
  if (!gmenu_exception() && !control_talk_last_key(v1) &&
      sgnTimeoutCurs == CURSOR_NONE && !deathflag) {
    if ((_BYTE)v1 == 'p' || (_BYTE)v1 == 'P') {
      ...
      case 'r':
        ...
        sprintf(tempstr, "Mid1 = %i : Mid2 = %i : Mid3 = %i",
                glMid1Seed[currlevel], glMid2Seed[currlevel],
      ...
      case 't':
        if (debug_mode_key_inverted_v) {
          sprintf(tempstr, "PX = %i  PY = %i", plr[myplr].WorldX,
                  plr[myplr].WorldY);
          NetSendCmdString(1 << myplr, tempstr);
          sprintf(tempstr, "CX = %i  CY = %i  DP = %i", cursmx, cursmy,
                  dungeon[cursmx][cursmy]);
          NetSendCmdString(1 << myplr, tempstr);
        }
        return;
      case '|':
...
void __fastcall LoadGameLevel(bool firstflag, int lvldir) {
  ...
  InitMultiView();
  IncProgress();
  if (firstflag || lvldir == 4 ||
      !plr[myplr]._pSLvlVisited[(unsigned char)setlvlnum]) {
    InitItems();
    SavePreLighting();
  } else {
    LoadLevel();
