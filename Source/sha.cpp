...
//----- (00456A16) --------------------------------------------------------
void __cdecl SHA1Clear() { memset(sgSHA1, 0, 0x114u); }
...
//----- (00456A4D) --------------------------------------------------------
void __fastcall SHA1Calculate(int n, const char *data,
...
//----- (00456A73) --------------------------------------------------------
void __fastcall SHA1Input(SHA1Context *context, const char *message_array,
                          int len) {
  SHA1Context *v3; // esi
  const char *v4;  // ebx
  int v5;          // ecx
...
void __fastcall SHA1ProcessMessageBlock(SHA1Context *context) {
  ...
  for (i = 40; i < 60; i++) {
    temp = SHA1CircularShift(5, A) + ((B & C) | (B & D) | (C & D)) + E + W[i] +
           0x8F1BBCDC;
    E = D;
    D = C;
    C = SHA1CircularShift(30, B);
