...
TSFX sgSFX[858] = {{2u, "Sfx\\Misc\\Walk1.wav", NULL},
...
void __fastcall InitMonsterSND(int monst) {
  ...
  if (gbSndInited) {
    ...
    do {
      if (monster_action_sounds[v1] != 's' ||
          *(int *)((char *)&monsterdata[0].snd_special + v9)) {
        v2 = 0;
        v3 = &Monsters[0].Snds[2 * (v1 + 41 * v8)];
        do {
          v4 = v2 + 1;
          sprintf(v7, *(const char **)((char *)&monsterdata[0].sndfile + v9),
                  monster_action_sounds[v1], v2 + 1);
          v5 = strlen(v7);
        ...
        if (!gbBufferMsgs) {
          ...
          if (v7) {
            ...
            if (!snd_playing(Monsters[0].Snds[v6])) {
              if (calc_snd_position(monster[v5]._mx, monster[v5]._my,
                                    &volume_delta, &pan))
...
//----- (00415ED8) --------------------------------------------------------
void __cdecl sound_init() { priv_sound_init(4); }
