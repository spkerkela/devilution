...
struct pfile_cpp_init {
  pfile_cpp_init() { pfile_cpp_init_value = pfile_inf; }
} _pfile_cpp_init;
...
void __cdecl pfile_init_save_directory() {
  ...
  if (GetWindowsDirectoryA(Buffer, 0x104u) &&
      (pfile_check_available_space(Buffer),
       GetModuleFileNameA(ghInst, Buffer, 0x104u))) {
    pfile_check_available_space(Buffer);
  } else {
...
void __fastcall pfile_check_available_space(char *pszDir) {
  ...
  v4 = GetDiskFreeSpaceA(v1, &SectorsPerCluster, &BytesPerSector,
                         &NumberOfFreeClusters, &TotalNumberOfClusters);
  if (!v4)
    goto LABEL_12;
  if ((signed __int64)(BytesPerSector * (unsigned __int64)SectorsPerCluster *
                       NumberOfFreeClusters) < 0xA00000)
    v4 = 0;
  if (!v4)
  LABEL_12:
    DiskFreeDlg(v1);
...
//----- (00449D43) --------------------------------------------------------
void __fastcall game_2_ui_player(PlayerStruct *p, _uiheroinfo *heroinfo,
                                 bool bHasSaveFile) {
  _uiheroinfo *v3;  // esi
  PlayerStruct *v4; // edi
  char v5;          // al
...
//----- (00449DE3) --------------------------------------------------------
bool __stdcall pfile_ui_set_hero_infos(
    void(__stdcall *ui_add_hero_info)(_uiheroinfo *)) {
  char *v1; // esi
  // int v2; // eax
  int v3;          // eax
  DWORD v4;        // eax
  ...
  if ((unsigned char)gbMaxPlayers > 1u) {
    ...
    do {
      ...
      if (v1 != (char *)1 && OpenFile(FileName, &ReOpenBuff, 0x4000u) != -1) {
        if (!SRegLoadString("Diablo\\Converted", (const char *)v1, 0,
                            NewFileName, 260)) {
          while (1) {
            v3 = save_num++;
    ...
    if (SFileReadFile(file, v5, v4, (unsigned long *)&dwBytes, 0)) {
      ...
      if ((unsigned char)gbMaxPlayers > 1u) {
        ...
        if (!SFileSetFilePointer(file, 0, 0, 0)) {
          //_LOBYTE(v8) = SFileReadFile(file, v6, v4, (unsigned long *)&dwBytes,
          //0);
...
//----- (0044A1CC) --------------------------------------------------------
bool __stdcall pfile_ui_set_class_stats(int player_class_nr,
                                        _uidefaultstats *class_stats) {
  int v2; // eax
...
//----- (0044A679) --------------------------------------------------------
void __fastcall pfile_write_save_file(char *pszName, void *pbData, int dwLen,
                                      int qwLen) {
  void *v4; // ebx
  int v5;   // eax
  // int v6; // eax
...
//----- (0044A727) --------------------------------------------------------
void __fastcall pfile_strcpy(char *dst, char *src) { strcpy(dst, src); }
...
char *__fastcall pfile_read(char *pszName, int *pdwLen) {
  ...
  *v2 = v6;
  ...
  src_dst = v7;
  //_LOBYTE(v8) = SFileReadFile(file, (char *)v7, *v2, (unsigned long *)&nread,
  //0);
