...
event_type event_types[3] = {EVENT_TYPE_PLAYER_LEAVE_GAME,
                             EVENT_TYPE_PLAYER_CREATE_GAME,
                             EVENT_TYPE_PLAYER_MESSAGE};

//----- (0043FAC9) --------------------------------------------------------
struct multi_cpp_init {
  multi_cpp_init() { multi_cpp_init_value = multi_inf; }
} _multi_cpp_init;
...
//----- (0043FD27) --------------------------------------------------------
unsigned char *__fastcall multi_recv_packet(void *packet, unsigned char *a2,
                                            int *a3) {
  char *v3;              // esi
...
//----- (0043FD90) --------------------------------------------------------
void __fastcall multi_send_msg_packet(int a1, unsigned char *a2,
                                      unsigned char len) {
  // const void *v3; // edx
  signed int v4;   // ebx
  unsigned int v5; // edi
  ...
  while (1) {
    if (v4 & v8) {
      if (!SNetSendMessage(v5, &pkt.hdr, len + 19) &&
          SErrGetLastError() != STORM_ERROR_INVALID_PLAYER)
        break;
    }
    ++v5;
    v4 *= 2;
...
int __fastcall multi_check_pkt_valid(char *a1) { return *(_DWORD *)a1 == 0; }
...
#endif
  {
    ...
    if (v1 <= 20000) {
      if (v1 >= 10000) {
        ...
        if (bGroupPlayers >= v6 &&
            (bGroupPlayers != v6 || nLowestPlayer == nLowestActive)) {
...
void __cdecl multi_process_network_packets() {
  ...
  if (SNetReceiveMessage((int *)arglist, (char **)&pkt, &len)) {
    do {
      ...
      v1 = pkt;
      v2 = pkt;
      if ((unsigned int)len >= 0x13 && *(_DWORD *)arglist < 4u &&
          pkt->wCheck == 'ip' && (unsigned short)pkt->wLen == len) {
        v3 = *(_DWORD *)arglist;
        ...
        if (!v4) {
          ...
          if (!v4 && plr[v3].plractive && plr[v3]._pHitPoints) {
            if (currlevel != plr[v3].plrlevel || plr[v3]._pLvlChanging) {
              ...
              if ((v6 > 3 || v7 > 3) &&
                  !dPlayer[(unsigned char)v2->px][(unsigned char)*v5]) {
              ...
              v11 = abs(plr[*(_DWORD *)arglist]._px -
                        plr[*(_DWORD *)arglist].WorldX);
              v12 = abs(plr[*(_DWORD *)arglist]._py -
                        plr[*(_DWORD *)arglist].WorldY);
              v13 = *(_DWORD *)arglist;
              if (v11 > 1 || v12 > 1) {
                v14 = *(_DWORD *)arglist;
              ...
              MakePlrPath(v13, (unsigned char)v2->targx,
                          (unsigned char)v2->targy, 1u);
            }
          }
        }
...
//----- (00440477) --------------------------------------------------------
void __fastcall multi_send_zero_packet(int pnum, char a2, void *pbSrc,
                                       int dwLen) {
  unsigned int v4;       // edi
  short v5;              // si
  ...
  while (v4) {
    ...
    *(_WORD *)&pkt.body[3] = dwBody;
    ...
    if (!SNetSendMessage(pnuma, &pkt.hdr,
                         *(unsigned short *)&pkt.body[3] + 24)) {
...
int __fastcall NetInit(int bSinglePlayer, int *pfExitProgram) {
  ...
  //_LOBYTE(v10) = SNetGetGameInfo(GAMEINFO_PASSWORD, szPlayerDescript, 128,
  //len);
...
//----- (00440A9B) --------------------------------------------------------
int __fastcall multi_init_single(_SNETPROGRAMDATA *client_info,
  ...
  //_LOBYTE(v3) = SNetInitializeProvider(0, client_info, user_info, ui_info,
  //&fileinfo);
  if (SNetInitializeProvider(0, client_info, user_info, ui_info, &fileinfo)) {
    ui_info = 0;
    //_LOBYTE(v5) = SNetCreateGame("local", "local", "local", 0, (char
    //*)&sgGameInitInfo.dwSeed, 8, 1, "local", "local", (int *)&ui_info);
    if (!SNetCreateGame("local", "local", "local", 0,
                        (char *)&sgGameInitInfo.dwSeed, 8, 1, "local", "local",
                        (int *)&ui_info)) {
      v6 = GetLastErr();
...
//----- (00440B09) --------------------------------------------------------
int __fastcall multi_init_multi(_SNETPROGRAMDATA *client_info,
                                _SNETPLAYERDATA *user_info,
                                _SNETUIDATA *ui_info, int *a4) {
  ...
  for (i = 1;; i = 0) {
    type = 0;
    if (byte_678640) {
      if (!UiSelectProvider(0, (_SNETPROGRAMDATA *)a2, v4, ui_info, &fileinfo,
                            &type) &&
          (!i || SErrGetLastError() != STORM_ERROR_REQUIRES_UPGRADE ||
           !multi_upgrade(a4))) {
        return 0;
      }
      if (type == 'BNET')
...
void __fastcall multi_player_joins(int pnum, TCmdPlrInfoHdr *cmd, int a3) {
  ...
  if (myplr != pnum) {
    ...
    if (*v5 == cmd->wOffset || (*v5 = 0, !cmd->wOffset)) {
      ...
      memcpy((char *)&pkplr[v3] + (unsigned short)v4->wOffset, &v4[1],
             (unsigned short)v4->wBytes);
      *v5 += v4->wBytes;
      if (*v5 == 1266) {
        *v5 = 0;
