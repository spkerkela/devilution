...
// but even this way gives nice indication of equal byte pairs, with
// significantly smaller size of the array that holds numbers of those hashes
...
static void SortBuffer(TCmpStruct *pWork, unsigned char *buffer_begin,
  ...
  unsigned short
      byte_pair_offs; // Offset of the byte pair, relative to "work_buff"
  ...
  //  ...
  //  offs 0x8F7: Number of occurences of PAIR_HASH 0x8F7 (the highest hash
  //  value)
  ...
  for (phash_to_index = pWork->phash_to_index;
       phash_to_index < &pWork->phash_to_index_end; phash_to_index++) {
...
static void OutputBits(TCmpStruct *pWork, unsigned int nbits,
...
static unsigned int FindRep(TCmpStruct *pWork, unsigned char *input_data) {
  ...
  unsigned short
      *phash_offs; // Pointer to the table containing offsets of each PAIR_HASH
  unsigned char *repetition_limit; // An eventual repetition must be at position
                                   // below this pointer
  unsigned char *prev_repetition;  // Pointer to the previous occurence of the
                                   // current PAIR_HASH
  unsigned char *prev_rep_end;     // End of the previous repetition
  unsigned char *input_data_ptr;
  unsigned short
      phash_offs_index;          // Index to the table with PAIR_HASH positions
  unsigned short min_phash_offs; // The lowest allowed hash offset
  unsigned short offs_in_rep;    // Offset within found repetition
  unsigned int equal_byte_count; // Number of bytes that are equal to the
                                 // previous occurence
  ...
  min_phash_offs = (unsigned short)((input_data - pWork->work_buff) -
                                    pWork->dsize_bytes + 1);
  ...
  for (;;) {
    ...
    if (*input_data_ptr == *prev_repetition &&
        input_data_ptr[rep_length - 1] == prev_repetition[rep_length - 1]) {
      ...
      if (equal_byte_count >= rep_length) {
        ...
        pWork->distance =
            (unsigned int)(input_data - prev_repetition + equal_byte_count - 1);
    ...
    // If the next repetition is beyond the minimum allowed repetition, we are
    // done.
    if (prev_repetition >= repetition_limit) {
  ...
  //          - Repetition of 0x1C bytes from [0x10]
  //          "EEEEEEEEEEEEEEEEQQQQQQQQQQQQ" The last repetition is the best
  //          one.
  //
  ...
  for (;;) {
    ...
    if (pre_last_byte == prev_repetition[rep_length - 2]) {
      ...
      do {
        ...
        if (prev_repetition >= repetition_limit)
          return rep_length;
      } while (prev_repetition[rep_length - 2] != pre_last_byte ||
               prev_repetition[0] != input_data[0]);
...
static void WriteCmpData(TCmpStruct *pWork) {
  ...
  unsigned int input_data_ended =
      0; // If 1, then all data from the input stream have been already loaded
  ...
  while (input_data_ended == 0) {
    ...
    while (bytes_to_load != 0) {
      bytes_loaded = pWork->read_buf(
          (char *)pWork->work_buff + pWork->dsize_bytes + 0x204 + total_loaded,
          &bytes_to_load, pWork->param);
      if (bytes_loaded == 0) {
    ...
    //
    // Warning: The end of the buffer passed to "SortBuffer" is actually 2 bytes
    // beyond valid data. It is questionable if this is actually a bug or not,
    // but it might cause the compressed data output to be dependent on random
    // bytes that are in the buffer. To prevent that, the calling application
    // must always zero the compression buffer before passing it to "implode"
    //
    ...
    case 1:
      SortBuffer(pWork, input_data - pWork->dsize_bytes + 0x204,
                 input_data_end + 1);
      phase++;
      break;
    ...
    while (input_data < input_data_end) {
      ...
      while (rep_length != 0) {
        ...
        if (input_data_ended && input_data + rep_length > input_data_end) {
          ...
          // If we got repetition of 2 bytes, that is 0x100 or more backward,
          // don't bother
          if (rep_length == 2 && pWork->distance >= 0x100)
            break;
        ...
        // returns the occurence of "AR". But there is longer repetition
        // "ROCKFORT", beginning 1 byte after.
        save_rep_length = rep_length;
        save_distance = pWork->distance;
        rep_length = FindRep(pWork, input_data + 1);

        // Only use the new repetition if it's length is greater than the
        // previous one
        if (rep_length > save_rep_length) {
          // If the new repetition if only 1 byte better
          // and the previous distance is less than 0x80 bytes, use the previous
          // repetition
          if (rep_length > save_rep_length + 1 || save_distance > 0x80) {
            // Flush one byte, so that input_data will point to the secondary
            // repetition
            OutputBits(pWork, pWork->nChBits[*input_data],
                       pWork->nChCodes[*input_data]);
            input_data++;
            continue;
          }
        }
      ...
      __FlushRepetition:
        ...
        OutputBits(pWork, pWork->nChBits[rep_length + 0xFE],
                   pWork->nChCodes[rep_length + 0xFE]);
        if (rep_length == 2) {
      ...
      // If there was no previous repetition for the current position in the
      // input data, just output the 9-bit literal for the one character
      OutputBits(pWork, pWork->nChBits[*input_data],
                 pWork->nChCodes[*input_data]);
      input_data++;
    _00402252:;
    ...
    if (input_data_ended == 0) {
      input_data -= 0x1000;
      memmove(pWork->work_buff, pWork->work_buff + 0x1000,
              pWork->dsize_bytes + 0x204);
    }
  }
...
unsigned int PKEXPORT
implode(unsigned int (*read_buf)(char *buf, unsigned int *size, void *param),
    ...
    if (1 << ExLenBits[i]) {
      for (nCount2 = 0; nCount2 < (1 << ExLenBits[i]); nCount2++) {
        ...
        pWork->nChCodes[nCount] =
            (unsigned short)((nCount2 << (LenBits[i] + 1)) |
                             ((LenCode[i] & 0xFFFF00FF) * 2) | 1);
        nCount++;
      }
    }
  }
