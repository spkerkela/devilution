...
typedef struct {
  unsigned int distance;   // 0000: Backward distance of the currently found
                           // repetition, decreased by 1
  unsigned int out_bytes;  // 0004: # bytes available in out_buff
  unsigned int out_bits;   // 0008: # of bits available in the last out byte
  unsigned int dsize_bits; // 000C: Number of bits needed for dictionary size. 4
                           // = 0x400, 5 = 0x800, 6 = 0x1000
  unsigned int dsize_mask; // 0010: Bit mask for dictionary. 0x0F = 0x400, 0x1F
                           // = 0x800, 0x3F = 0x1000
  ...
  unsigned char nChBits[0x306]; // 009C: Table of literal bit lengths to be put
                                // to the output stream
  unsigned short nChCodes[0x306]; // 03A2: Table of literal codes to be put to
                                  // the output stream
  ...
  unsigned short
      phash_to_index[0x900]; // 0DC8: Array of indexes (one for each PAIR_HASH)
                             // to the "pair_hash_offsets" table
  unsigned short
      phash_to_index_end; // 1FC8: End marker for "phash_to_index" table
  ...
  unsigned short
      phash_offs[0x2204]; // 49D0: Table of offsets for each PAIR_HASH
} TCmpStruct;
...
typedef struct {
  ...
  unsigned long dsize_mask; // 0010: Dict size bitmask (0x0F, 0x1F, 0x3F for
                            // 0x400, 0x800, 0x1000)
  ...
  unsigned char
      out_buff[0x2204]; // 0030: Output circle buffer.
                        //       0x0000 - 0x0FFF: Previous uncompressed data,
                        //       kept for repetitions 0x1000 - 0x1FFF: Currently
                        //       decompressed data 0x2000 - 0x2203: Reserve
                        //       space for the longest possible repetition
  ...
  unsigned char
      DistBits[0x40]; // 30B4: Numbers of bytes to skip copied block length
  unsigned char
      LenBits[0x10]; // 30F4: Numbers of bits for skip copied block length
