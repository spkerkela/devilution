...
BOOL STORMAPI SNetCreateGame(const char *pszGameName,
                             ...
                             const char *pszGameStatString, DWORD dwGameType,
                             char *GameTemplateData, int GameTemplateSize,
                             int playerCount, char *creatorName, char *a11,
                             int *playerID);
 ...
 *  Returns TRUE if the function was called successfully and FALSE otherwise.
 */
BOOL STORMAPI SNetGetGameInfo(int type, void *dst, size_t length,
...
BOOL STORMAPI SNetGetNumPlayers(int *firstplayerid, int *lastplayerid,
                                int *activeplayers);
...
typedef struct _CAPS {
  ...
  DWORD dwDisplayedPlayerCount; // Displayed player count in the mode selection
                                // list
  DWORD dwUnk_0x14;             // some kind of timeout or timer related
  DWORD dwPlayerLatency;        // ... latency?
  DWORD dwPlayerCount; // the number of players that can participate, must be
                       // between 1 and 20
  DWORD dwCallDelay; // the number of calls before data is sent over the network
                     // // between 2 and 8; single player is set to 1
} CAPS, *PCAPS;
 ...
 *
 *  providerCaps: A pointer to a CAPS structure that will receive the
 * information.
 *
 *  Returns TRUE if the function was called successfully and FALSE otherwise.
 */
...
BOOL STORMAPI SNetJoinGame(int id, char *gameName, char *gamePassword,
                           char *playerName, char *userStats, int *playerid);
 ...
 *
 *  type: The leave type. It doesn't appear to be important, no documentation
 * available.
 *
 *  Returns TRUE if the function was called successfully and FALSE otherwise.
 */
...
BOOL STORMAPI SNetReceiveMessage(int *senderplayerid, char **data,
                                 int *databytes);
BOOL STORMAPI SNetReceiveTurns(int a1, int arraysize, char **arraydata,
...
// HANDLE STORMAPI SNetRegisterEventHandler(int type, void (STORMAPI
// *sEvent)(PS_EVT));

int STORMAPI SNetSelectGame(int a1, int a2, int a3, int a4, int a5,
                            int *playerid);
 ...
 *                  SNPLAYER_ALL      | Sends the message to all players,
 * including oneself. SNPLAYER_OTHERS   | Sends the message to all players,
 * except for oneself. data:       A pointer to the data. databytes:  The amount
 * of bytes that the data pointer contains.
 *
 *  Returns TRUE if the function was called successfully and FALSE otherwise.
 */
 ...
 *  makePublic: Used to make the game a public game, removing the
 * GAMESTATE_PRIVATE flag.
 *
 *  Returns TRUE if the function was called successfully and FALSE otherwise.
 */
BOOL STORMAPI SNetSetGameMode(DWORD modeFlags, bool makePublic = false);

#define SNMakeGamePublic()                                                     \
  SNetSetGameMode(                                                             \
      (DWORD mode, SNetGetGameInfo(GAMEINFO_MODEFLAGS, &mode, 4), mode), true)

BOOL STORMAPI SNetEnumGamesEx(int a1, int a2,
...
BOOL STORMAPI SNetCreateLadderGame(
    const char *pszGameName, const char *pszGamePassword,
    const char *pszGameStatString, DWORD dwGameType, DWORD dwGameLadderType,
    DWORD dwGameModeFlags, char *GameTemplateData, int GameTemplateSize,
    int playerCount, char *creatorName, char *a11, int *playerID);
...
BOOL STORMAPI SNetReportGameResult(unsigned a1, int size, int *results,
...
HGDIOBJ STORMAPI SDlgDefDialogProc(HWND hDlg, signed int DlgType, HDC textLabel,
                                   HWND hWnd);

HANDLE STORMAPI SDlgDialogBoxIndirectParam(HMODULE hModule, LPCSTR lpName,
                                           HWND hWndParent, LPVOID lpParam,
                                           LPARAM lParam);

BOOL STORMAPI SDlgEndDialog(HWND hDlg, HANDLE nResult);

BOOL STORMAPI SDlgSetControlBitmaps(HWND parentwindow, int *id, int a3,
                                    char *buffer2, char *buffer, int flags,
                                    int mask);

/*
// lpCursorName can only be IDC_ARROW
BOOL STORMAPI SDlgSetSystemCursor(void *lpSrcBuffer, void *p_a2, LPSIZE lpSize,
LPCSTR lpCursorName);
*/

BOOL STORMAPI SDlgBltToWindowE(HWND hWnd, HRGN a2, char *a3, int a4,
                               void *buffer, RECT *rct, SIZE *size, int a8,
                               int a9, DWORD rop);
BOOL STORMAPI SDlgSetBitmapE(HWND hWnd, int a2, char *src, int mask1, int flags,
                             int a6, int a7, int width, int a9, int mask2);
...
BOOL STORMAPI SFileDdaBeginEx(HANDLE directsound, DWORD flags, DWORD mask,
...
BOOL STORMAPI SFileDdaSetVolume(HANDLE directsound, signed int bigvolume,
                                signed int volume);
...
BOOL STORMAPI SFileOpenArchive(const char *szMpqName, DWORD dwPriority,
...
BOOL STORMAPI SFileOpenFileEx(HANDLE hMpq, const char *szFileName,
...
BOOL STORMAPI SFileReadFile(HANDLE hFile, void *buffer,
                            DWORD nNumberOfBytesToRead, DWORD *read,
...
// BOOL STORMAPI callback(const char *pszFilename, DWORD dwErrCode, DWORD
// dwErrCount)
BOOL STORMAPI SFileSetIoErrorMode(DWORD mode,
                                  BOOL(STORMAPI *callback)(const char *, DWORD,
                                                           DWORD));
...
BOOL STORMAPI SFileLoadFile(char *filename, void *buffer, int buffersize,
                            int a4, int a5);
BOOL STORMAPI SFileUnloadFile(HANDLE hFile);
BOOL STORMAPI SFileLoadFileEx(void *hArchive, char *filename, int a3, int a4,
                              int a5, DWORD searchScope,
                              struct _OVERLAPPED *lpOverlapped);

// Options are DWORD except for #6
// 1: [TRUE|FALSE] - If true, reports resource leaks
// (SErrReportResourceLeak/SErrReportNamedResourceLeak) to the attached debugger
// instead of a message box. 2: This option is unused. 3: [TRUE|FALSE] - If
// true, reports general memory leaks to the attached debugger instead of a
// message box. 4: This option is unused. 5: [TRUE|FALSE] - If true, reports log
// messages and log dumps to the attached debugger. 6: { DWORD blocks_allocated;
// DWORD blocks_freed; } Used to determine the amount of memory/heap blocks that
// have been allocated and freed by storm.
...
BOOL STORMAPI SBltROP3(void *lpDstBuffer, void *lpSrcBuffer, int srcDrawWidth,
                       int srcDrawHeight, int dstWidth, int srcWidth, int a7,
                       DWORD rop);
BOOL STORMAPI SBltROP3Clipped(void *lpDstBuffer, RECT *lpDstRect,
                              POINT *lpDstPt, int a4, void *lpSrcBuffer,
                              RECT *lpSrcRect, POINT *lpSrcPt, int a8, int a9,
                              DWORD rop);
 ...
 *                    destination buffer is 0, then the destination buffer is
 * not used. pdwWidth:         An optional variable that receives the image
 * width. pdwHeight:        An optional variable that receives the image height.
 *  pdwBpp:           An optional variable that receives the image bits per
 * pixel.
 *
 *  Returns TRUE if the image was supported and decoded correctly, FALSE
 * otherwise.
 */
BOOL STORMAPI SBmpDecodeImage(DWORD dwImgType, void *pSrcBuffer,
                              DWORD dwSrcBuffersize,
                              PALETTEENTRY *pPalette = NULL,
                              void *pDstBuffer = NULL,
                              DWORD dwDstBuffersize = 0, DWORD *pdwWidth = NULL,
                              DWORD *pdwHeight = NULL, DWORD *pdwBpp = NULL);
 ...
 *
 *  Returns TRUE if the image was supported and loaded correctly, FALSE
 * otherwise.
 */
BOOL STORMAPI SBmpLoadImage(const char *pszFileName,
                            PALETTEENTRY *pPalette = NULL, void *pBuffer = NULL,
                            DWORD dwBuffersize = 0, DWORD *pdwWidth = NULL,
                            DWORD *pdwHeight = NULL, DWORD *pdwBpp = NULL);
 ...
 */
BOOL STORMAPI SBmpSaveImage(const char *pszFileName, PALETTEENTRY *pPalette,
                            void *pBuffer, DWORD dwWidth, DWORD dwHeight,
                            DWORD dwBpp = 8);

HANDLE STORMAPI SBmpAllocLoadImage(const char *fileName, PALETTEENTRY *palette,
                                   void **buffer, int *width, int *height,
                                   int unused6, int unused7,
                                   void *(STORMAPI *allocFunction)(DWORD));

BOOL STORMAPI SCodeCompile(char *directives1, char *directives2,
                           char *loopstring, unsigned int maxiterations,
                           unsigned int flags, HANDLE handle);
BOOL STORMAPI SCodeDelete(HANDLE handle);

int STORMAPI SCodeExecute(HANDLE handle, int a2);

BOOL STORMAPI SDrawAutoInitialize(HINSTANCE hInst, LPCSTR lpClassName,
                                  LPCSTR lpWindowName, WNDPROC pfnWndProc,
                                  int nMode, int nWidth, int nHeight,
                                  int nBits);
 ...
 *
 *  pszOutput: The name of the output file. The save format is automatically set
 * by the extension. The extensions supported are .gif, .pcx, .tga, and .bmp. It
 * will write a bitmap by default.
 *
 ...
 */
HWND STORMAPI SDrawGetFrameWindow(HWND *sdraw_framewindow = NULL);
 ...
 */
BOOL STORMAPI SDrawGetObjects(LPDIRECTDRAW *ddInterface = NULL,
 ...
 */
BOOL STORMAPI SDrawGetScreenSize(DWORD *pdwWidth, DWORD *pdwHeight,
                                 DWORD *pdwBpp);

// undefined
BOOL STORMAPI SDrawLockSurface(int surfacenumber, RECT *lpDestRect,
                               void **lplpSurface, int *lpPitch,
                               int arg_unused);
 ...
 *  Returns FALSE if no variables were specified.
 */
BOOL STORMAPI SDrawManualInitialize(HWND hWnd = NULL,
...
BOOL STORMAPI SDrawUnlockSurface(int surfacenumber, void *lpSurface, int a3,
                                 RECT *lpRect);
BOOL STORMAPI SDrawUpdatePalette(unsigned int firstentry,
                                 unsigned int numentries,
                                 PALETTEENTRY *pPalEntries, int a4);

BOOL STORMAPI SEvtDispatch(DWORD dwMessageID, DWORD dwFlags, int type,
                           PS_EVT pEvent);

BOOL STORMAPI SGdiDeleteObject(HANDLE handle);

BOOL STORMAPI SGdiExtTextOut(int a1, int a2, int a3, int a4, unsigned int a8,
                             signed int a6, signed int a7,
                             const char *pszString, unsigned int arg20);
 ...
 */
void *STORMAPI SMemAlloc(size_t amount, char *logfilename, int logline,
                         char defaultValue = 0);
 ...
 */
BOOL STORMAPI SMemFree(void *location, char *logfilename, int logline,
                       char defaultValue = 0);
 ...
 *                is NULL, then SMemAlloc is called with the remaining
 * parameters. amount:       The amount of memory to re-allocate. logfilename:
 * The name of the file or object that this call belongs to. logline:      The
 * line in the file or one of the SLOG_ macros. defaultValue:
 *
 *  Returns a pointer to the re-allocated memory. This pointer does NOT include
 *  the additional storm header.
 */
void *STORMAPI SMemReAlloc(void *location, size_t amount, char *logfilename,
                           int logline, char defaultValue = 0);

#define SMReAlloc(loc, s) SMemReAlloc((loc), (s), __FILE__, __LINE__)

// Can be provided instead of logline/__LINE__ parameter to indicate different
// errors.
...
BOOL STORMAPI SRegLoadData(const char *keyname, const char *valuename, int size,
                           LPBYTE lpData, BYTE flags, LPDWORD lpcbData);
BOOL STORMAPI SRegLoadString(const char *keyname, const char *valuename,
                             BYTE flags, char *buffer, size_t buffersize);
BOOL STORMAPI SRegLoadValue(const char *keyname, const char *valuename,
                            BYTE flags, int *value);
BOOL STORMAPI SRegSaveData(const char *keyname, const char *valuename, int size,
                           BYTE *lpData, DWORD cbData);
BOOL STORMAPI SRegSaveString(const char *keyname, const char *valuename,
                             BYTE flags, char *string);
BOOL STORMAPI SRegSaveValue(const char *keyname, const char *valuename,
                            BYTE flags, DWORD result);

BOOL STORMAPI SRegDeleteValue(const char *keyname, const char *valuename,
                              BYTE flags);
...
#define SREG_EXCLUDE_LOCAL_MACHINE                                             \
  0x00000001 // excludes checking the HKEY_LOCAL_MACHINE hive
#define SREG_BATTLE_NET                                                        \
  0x00000002 // sets the relative key to "Software\\Battle.net\\" instead
#define SREG_EXCLUDE_CURRENT_USER                                              \
  0x00000004                     // excludes checking the HKEY_CURRENT_USER hive
#define SREG_ABSOLUTE 0x00000010 // specifies that the key is not a relative key

BOOL STORMAPI STransBlt(void *lpSurface, int x, int y, int width,
                        HANDLE hTrans);
BOOL STORMAPI STransBltUsingMask(void *lpDest, void *lpSource, int pitch,
                                 int width, HANDLE hTrans);

BOOL STORMAPI STransDelete(HANDLE hTrans);

BOOL STORMAPI STransDuplicate(HANDLE hTransSource, HANDLE hTransDest);
BOOL STORMAPI STransIntersectDirtyArray(HANDLE hTrans, char *dirtyarraymask,
                                        unsigned flags, HANDLE *phTransResult);
BOOL STORMAPI STransInvertMask(HANDLE hTrans, HANDLE *phTransResult);

BOOL STORMAPI STransSetDirtyArrayInfo(int width, int height, int depth,
                                      int bits);

BOOL STORMAPI STransPointInMask(HANDLE hTrans, int x,
                                int y); // Name is a pure guess
BOOL STORMAPI STransCombineMasks(HANDLE hTransA, HANDLE hTransB, int left,
                                 int top, int flags, HANDLE *phTransResult);

BOOL STORMAPI STransCreateE(void *pBuffer, int width, int height, int bpp,
                            int a5, int bufferSize, HANDLE *phTransOut);
...
BOOL STORMAPI SVidPlayBegin(char *filename, int arg4, int a3, int a4, int a5,
                            int a6, HANDLE *video);
 ...
 *
 * Displays a formatted error message. The message is detailed and flexible for
 * many applications. The message will be different if there is a debugger
 * attached. Will typically terminate the application unless the option to
 * continue is given.
 *
 ...
 *  allowOption:    If TRUE, allows the user the option to continue execution,
 * otherwise the program will terminate. exitCode:       The exit code used for
 * program termination.
 *
 *  Returns TRUE if the user chose to continue execution, FALSE otherwise.
 */
BOOL STORMAPI SErrDisplayError(DWORD dwErrMsg, const char *logfilename,
                               int logline, const char *message = NULL,
                               BOOL allowOption = FALSE, int exitCode = 1);
 ...
 *  Returns TRUE if the call was successful and FALSE otherwise.
 */
BOOL STORMAPI SErrGetErrorStr(DWORD dwErrCode, char *buffer,
                              size_t bufferchars);
...
// Registers a module as a message source for SErrGetErrorStr, always returns
// TRUE groupID is a group in a MessageTable entry for example in
// STORM_ERROR_BAD_ARGUMENT 0x85100065, 0x510 is the group. BOOL STORMAPI
// SErrRegisterMessageSource(WORD groupID, HMODULE hSourceModule, int a3)
 ...
 */
void STORMAPI SErrSetLastError(DWORD dwErrCode = NO_ERROR);

//
// void STORMAPI SErrReportNamedResourceLeak(const char *pszMsg, const char
// *pszSubMsg = nullptr) void STORMAPI SErrReportResourceLeak(const char
// *pszMsg)
 ...
 */
void STORMAPI SMemFill(void *location, size_t length, char fillWith = 0);
 ...
 */
int STORMAPI SStrCopy(char *dest, const char *src, int max_length = 0x7FFFFFFF);
...
DWORD
STORMAPI
SStrHash(const char *string, DWORD flags = 0, DWORD Seed = 0);
 ...
 *  allowOption:    If TRUE, allows the user the option to continue execution,
 * otherwise the program will terminate. exitCode:       The exit code used for
 * program termination. format:         Additional message formatting. See
 * printf.
 *
 *  Returns TRUE if the user chose to continue execution, FALSE otherwise.
 */
BOOL SErrDisplayErrorFmt(DWORD dwErrMsg, const char *logfilename, int logline,
                         BOOL allowOption, int exitCode, const char *format,
                         ...);

//#define SEDisplayErrorFmt(err,...) SErrDisplayErrorFmt(err, __FILE__,
//__LINE__, FALSE, 1, __VA_ARGS__)

/*  SErrCatchUnhandledExceptions @ 567
 *
 *  Registers a top-level exception filter managed entirely by Storm.
 *  The registered filter will display formatted exception information by
 * calling SErrDisplayError.
 */
...
void *__stdcall SNetUnregisterEventHandler(
    int, void(__stdcall *)(struct _SNETEVENT *));
void *__stdcall SNetRegisterEventHandler(
    int, void(__stdcall *)(struct _SNETEVENT *));
bool __stdcall SNetSetBasePlayer(int);
int __stdcall SNetInitializeProvider(unsigned long, struct _SNETPROGRAMDATA *,
